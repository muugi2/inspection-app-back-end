...
Write-Host "[7] 192.168.0.7:21 шалгаж байна..." -ForegroundColor Yellow
Write-Host ""
$test192 = Test-NetConnection -ComputerName "192.168.0.7" -Port 21 -WarningAction SilentlyContinue
if ($test192.TcpTestSucceeded) {
    Write-Host "   ✓ 192.168.0.7:21 холболт амжилттай!" -ForegroundColor Green
    Write-Host "   → Tablet-аас холбогдох боломжтой байх ёстой" -ForegroundColor Green
} else {
    Write-Host "   ✗ 192.168.0.7:21 холболт амжилтгүй" -ForegroundColor Red
...
Write-Host "  ftp 192.168.0.7" -ForegroundColor White
Write-Host ""
